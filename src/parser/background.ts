...
const bgImage: bnb.Parser<Image | undefined> = image.or(
  keyword("none").map(() => undefined),
);
...
const bgPosition1: bnb.Parser<Position> = keywords([
  ...
  .map((value) => {
    ...
    switch (value) {
      ...
      case "right":
        return new Position(
          { from: value, offset: new Dimension(0, "") },
          "center",
        );
      case "center":
...
const bgPosition34: bnb.Parser<Position> = doubleAmpersand({
  x: keyword("center").or(
    bnb.all(
      keywords(["left", "right"] as const),
      lengthPercentage.or(bnb.ok(new Dimension(0, ""))),
    ),
  ),
  y: keyword("center").or(
    bnb.all(
      keywords(["top", "bottom"] as const),
      lengthPercentage.or(bnb.ok(new Dimension(0, ""))),
    ),
  ),
}).map(({ x, y }) => {
...
export const bgPosition: bnb.Parser<Position> = bnb.choice(
  bgPosition34,
  bgPosition2,
  bgPosition1,
);
...
export const bgLayer: bnb.Parser<BackgroundLayer> = doubleBar({
  image: bgImage,
  positionSize: bnb.all(
    bgPosition,
    keyword("/").next(bgSize).or(bnb.ok(undefined)),
  ),
